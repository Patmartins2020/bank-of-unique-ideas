...
export default function Home() {
    ...
    async function loadAll() {
      ...
      try {
        ...
        const { data: ideasData, error: ideasErr } = await supabase
          .from('ideas')
          .select(
            'id,title,tagline,impact,category,status,protected,created_at'
          )
  ...
  const cats = useMemo(() => {
    return [
      'All',
      ...Array.from(new Set(ideas.map((i) => i.category))).filter(Boolean),
    ];
  }, [ideas]);
  ...
  return (
    <main className="min-h-screen px-6 py-12 text-center text-white bg-gradient-to-b from-neutral-950 via-slate-950 to-neutral-900">
      ...
      <p className="text-gray-400 max-w-xl mx-auto mb-10">
        A digital United Nations of inventions and innovations — where every
        mind counts.
      </p>
                ...
                >
                  <div className="flex items-center justify-between">
                    <h2
                      ...
                      title={
                        blurred
                          ? 'Title hidden — request NDA to view.'
                          : undefined
                      }
                    >
                      {idea.title}
                  ...
                  <p
                    ...
                    title={
                      blurred
                        ? 'Protected concept — request NDA to view details.'
                        : undefined
                    }
                  >
                    {idea.tagline || '—'}
                  ...
                  <p
                    ...
                    title={
                      blurred
                        ? 'Protected impact — available under NDA.'
                        : undefined
                    }
                  >
                    {idea.impact || '—'}
                    ...
                    {blurred ? (
                      <div className="group/tt inline-block">
                        ...
                        <div className="invisible group-hover/tt:visible absolute z-10 mt-2 w-64 rounded-md border border-white/15 bg-white/10 p-3 text-xs text-white/80 backdrop-blur">
                          This card is intentionally blurred to protect the
                          inventor’s IP. Click “Request NDA” to view the full
                          brief.
                        </div>
                      </div>
                    ) : (
